...
class DbFiller:
    ...
    def run(self):
        if not self.options.get('clean', False):
            ...
            print('Added {} health states'.format(len(self.institutions)))
        if self.options.get('clean', False):
            self._clean_db()

    def _clean_db(self):
        HealthStates.objects.all().delete()
        print('Database cleaned')

    def _get_institutions(self):
        institutions_text = open(os.path.join(ROOT_DIR, "fixtures", "health.json"), "r", encoding='utf-8').read()
        institutions = json.loads(institutions_text)
        for institution in institutions:
            try:
                HealthStates.objects.create(name=institution["name"])
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument('--create',
                            ...
                            default=True,
                            dest='institutions',
                            help='загрузка в справочник состояний здоровья')
