...
class DbFiller:
    ...
    def _get_institutions(self):
        institutions_text = open(os.path.join(ROOT_DIR, "fixtures", "groups.json"), "r", encoding='utf-8').read()
        institutions = json.loads(institutions_text)
        for institution in institutions:
            try:
                Group.objects.create(name=institution["name"])
...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument('--create',
                            ...
                            default=True,
                            dest='institutions',
                            help='загрузка в справочник групп дошкольных учреждений')
